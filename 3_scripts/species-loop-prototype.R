...
## 
## In theory, only material in "Parameters for looping" needs to be changed to specify
## new models, data, etc.
...
## gause activity curve static in each region
form.7 = formula(count ~ doy:regionfac + I(doy^2):regionfac +
                   te(lat, lon, by = year, k = c(5, 5), bs = c(cr, cr)) + sourcefac)
...
run.suffix = "baseline-constrainedgeo" ## Change this for whatever you're trying out
