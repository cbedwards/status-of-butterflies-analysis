...
if(specs.do.all){
  ...
  dat.sum = dat %>% 
    filter(presence == 1) %>% 
    filter(source == "Illinois Butterfly Monitoring Network") %>% 
    group_by(code) %>% 
...
problem.codes = c("ASTCEL", "HYLPHY", "JUNCOE", "PAPCRE", "VANVIR")

## debugging iteration:
# specs.do = specs.do[-(1:max(which(specs.do$code %in% problem.codes))),]

# problem.codes = c("CALLDUM", "CALLPOL", "CHLOPAL", "ERYZAR", "EUPYBIM")
# #### Plus additional problem species for this data set only:
# additional.problems = c("ASTCEL", "BOLOSEL", "HYLPHY", "JUNCOE", "PAPCRE", "PHOESEN", "POAVIA", "VANVIR")
# problem.codes = c(problem.codes, additional.problems)
